function [ImOut,imOut_addresses] = Imrotate5(ImIn,angle,ZoomFactor,Xstart,Ystart,VerResOut,HorResOut)
...
imOut_addresses=(zeros(XSizeIn,YSizeIn,p));
addr_i=1;
addr_j=1;
...
for t=(VerResOut-XSizeIn)/2:1:(VerResOut-XSizeIn)/2+XSizeIn
   for s=(HorResOut-YSizeIn)/2:1:(HorResOut-YSizeIn)/2+YSizeIn
...
%crop image
 ...
 % built image of addreses
 imOut_addresses(addr_i,addr_j)=j+(i-1)*YSizeIn;
 if addr_j<=YSizeIn 
     addr_j=addr_j+1;
 else 
     addr_j=1;
