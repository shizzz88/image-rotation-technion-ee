...
img=imread('../../General/Test Images/lena_512x384.jpg');
Img=zeros(1,512*384);
Img=reshape(img,1,512*384);%----------------need fix


ImOut=uint8(zeros(600,800));
% operating zoom, rotation, resolution change. 
%x=0;
row_start=(HorResOut-XSizeIn)/2;
col_start=(XSizeIn-YSizeIn)/2;
row_end=row_start+XSizeIn;
col_end=col_start+YSizeIn;
for row=row_start:row_end-1%:-1:1
   for col=col_start:col_end-1 %:-1:1
       index=col+(row-1)*800;
